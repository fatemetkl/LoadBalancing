...
public class MainScript2 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			Long[] runTimes = new Long[Constants.NUMBER_OF_BALANCING_ALGORITHMS * 2];
			for (int i = 0; i < Constants.NUMBER_OF_BALANCING_ALGORITHMS * 2; i++) {
			//for (int i = 0; i < 2; i++) {
				if(i % 2 == 0){
					Message setBalanceType = new Message(Constants.RequestType.SET_LOAD_LB, client.getId(), i / 2);
					client.sendMessage(setBalanceType);
				}
				if(i != 1){
					System.out.println("\n__________________________________________");
					System.out.println("Benchmarking on algorithm " + i / 2 + ":");
			...
			for (int i = 0; i < Constants.NUMBER_OF_BALANCING_ALGORITHMS * 2; i++) {
			//for (int i = 0; i < 2; i++) {
				System.out.println("Algorithm " + (i / 2) + " performance: " + runTimes[i] + " seconds.");
			} 
